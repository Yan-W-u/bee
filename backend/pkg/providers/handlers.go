...
func (fp *flowProvider) GetExplainerHandler(ctx context.Context, taskID, subtaskID *int64) (tools.ExecutorHandler, error) {
	ptrTask, ptrSubtask, err := fp.getTaskAndSubtask(ctx, taskID, subtaskID)
	if err != nil {
		return nil, err
	}
	...
	explainerHandler := func(ctx context.Context, action tools.ExplainerAction) (string, error) {
		explainerContext := map[string]map[string]any{
			"user": {
				"Question": action.Question,
			},
			"system": {
				"ExplainerResultToolName":  tools.ExplainerResultToolName,
				"SearchGuideToolName":      tools.SearchGuideToolName,
				"StoreGuideToolName":       tools.StoreGuideToolName,
				"GraphitiEnabled":          fp.graphitiClient != nil && fp.graphitiClient.IsEnabled(),
				"GraphitiSearchToolName":   tools.GraphitiSearchToolName,
		...
		explainerCtx, observation := obs.Observer.NewObservation(ctx)
		explainerEvaluator := observation.Evaluator(
			langfuse.WithEvaluatorName("render explainer agent prompts"),
			langfuse.WithEvaluatorInput(explainerContext),
			langfuse.WithEvaluatorMetadata(langfuse.Metadata{
				"user_context":   explainerContext["user"],
				"system_context": explainerContext["system"],
				"task":           ptrTask,
				"subtask":        ptrSubtask,
				"lang":           fp.language,
			}),
		...
		userExplainerTmpl, err := fp.prompter.RenderTemplate(templates.PromptTypeQuestionExplainer, explainerContext["user"])
		if err != nil {
			return "", wrapErrorEndEvaluatorSpan(explainerCtx, explainerEvaluator, "failed to get user explainer template", err)
		}

		systemExplainerTmpl, err := fp.prompter.RenderTemplate(templates.PromptTypeExplainer, explainerContext["system"])
		if err != nil {
			return "", wrapErrorEndEvaluatorSpan(explainerCtx, explainerEvaluator, "failed to get system explainer template", err)
		}

		explainerEvaluator.End(
			langfuse.WithEvaluatorOutput(map[string]any{
				"user_template":   userExplainerTmpl,
				"system_template": systemExplainerTmpl,
			}),
			langfuse.WithEvaluatorStatus("success"),
			langfuse.WithEvaluatorLevel(langfuse.ObservationLevelDebug),
		)

		explainerResult, err := fp.performExplainer(ctx, taskID, subtaskID, systemExplainerTmpl, userExplainerTmpl, action.Question)
		if err != nil {
			return "", wrapError(ctx, "failed to get explainer result", err)
		}

		return explainerResult, nil
	}

	return func(ctx context.Context, name string, args json.RawMessage) (string, error) {
		ctx, span := obs.Observer.NewSpan(ctx, obs.SpanKindInternal, "providers.flowProvider.getExplainerHandler")
		defer span.End()

		var action tools.ExplainerAction
		if err := json.Unmarshal(args, &action); err != nil {
			logrus.WithContext(ctx).WithError(err).Error("failed to unmarshal explainer payload")
			return "", fmt.Errorf("failed to unmarshal explainer payload: %w", err)
		}

		explainerResult, err := explainerHandler(ctx, action)
		if err != nil {
			return "", err
		}
...
func (fp *flowProvider) GetRemediatorHandler(ctx context.Context, taskID, subtaskID *int64) (tools.ExecutorHandler, error) {
	ptrTask, ptrSubtask, err := fp.getTaskAndSubtask(ctx, taskID, subtaskID)
	if err != nil {
		return nil, err
	}
	...
	remediatorHandler := func(ctx context.Context, action tools.RemediatorAction) (string, error) {
		remediatorContext := map[string]map[string]any{
			"user": {
				"Question": action.Question,
			},
			"system": {
				"RemediatorResultToolName":  tools.RemediatorResultToolName,
				"SearchGuideToolName":       tools.SearchGuideToolName,
				"StoreGuideToolName":        tools.StoreGuideToolName,
				"GraphitiEnabled":           fp.graphitiClient != nil && fp.graphitiClient.IsEnabled(),
				"GraphitiSearchToolName":    tools.GraphitiSearchToolName,
		...
		remediatorCtx, observation := obs.Observer.NewObservation(ctx)
		remediatorEvaluator := observation.Evaluator(
			langfuse.WithEvaluatorName("render remediator agent prompts"),
			langfuse.WithEvaluatorInput(remediatorContext),
			langfuse.WithEvaluatorMetadata(langfuse.Metadata{
				"user_context":   remediatorContext["user"],
				"system_context": remediatorContext["system"],
				"task":           ptrTask,
				"subtask":        ptrSubtask,
				"lang":           fp.language,
			}),
		...
		userRemediatorTmpl, err := fp.prompter.RenderTemplate(templates.PromptTypeQuestionRemediator, remediatorContext["user"])
		if err != nil {
			return "", wrapErrorEndEvaluatorSpan(remediatorCtx, remediatorEvaluator, "failed to get user remediator template", err)
		}

		systemRemediatorTmpl, err := fp.prompter.RenderTemplate(templates.PromptTypeRemediator, remediatorContext["system"])
		if err != nil {
			return "", wrapErrorEndEvaluatorSpan(remediatorCtx, remediatorEvaluator, "failed to get system remediator template", err)
		}

		remediatorEvaluator.End(
			langfuse.WithEvaluatorOutput(map[string]any{
				"user_template":   userRemediatorTmpl,
				"system_template": systemRemediatorTmpl,
			}),
			langfuse.WithEvaluatorStatus("success"),
			langfuse.WithEvaluatorLevel(langfuse.ObservationLevelDebug),
		)

		remediatorResult, err := fp.performRemediator(ctx, taskID, subtaskID, systemRemediatorTmpl, userRemediatorTmpl, action.Question)
		if err != nil {
			return "", wrapError(ctx, "failed to get remediator result", err)
		}

		return remediatorResult, nil
	}

	return func(ctx context.Context, name string, args json.RawMessage) (string, error) {
		ctx, span := obs.Observer.NewSpan(ctx, obs.SpanKindInternal, "providers.flowProvider.getRemediatorHandler")
		defer span.End()

		var action tools.RemediatorAction
		if err := json.Unmarshal(args, &action); err != nil {
			logrus.WithContext(ctx).WithError(err).Error("failed to unmarshal remediator payload")
			return "", fmt.Errorf("failed to unmarshal remediator payload: %w", err)
		}

		remediatorResult, err := remediatorHandler(ctx, action)
		if err != nil {
			return "", err
		}

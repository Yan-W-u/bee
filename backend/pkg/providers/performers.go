...
func (fp *flowProvider) performExplainer(
	ctx context.Context,
	taskID, subtaskID *int64,
	systemExplainerTmpl, userExplainerTmpl, question string,
) (string, error) {
	var (
		explainerResult tools.ExplainerResult
		optAgentType    = pconfig.OptionsTypeExplainer
		msgChainType    = database.MsgchainTypeExplainer
	)

	ctx = tools.PutAgentContext(ctx, msgChainType)
	cfg := tools.ExplainerExecutorConfig{
		TaskID:    taskID,
		SubtaskID: subtaskID,
		ExplainerResult: func(ctx context.Context, name string, args json.RawMessage) (string, error) {
			err := json.Unmarshal(args, &explainerResult)
			if err != nil {
				return "", fmt.Errorf("failed to unmarshal result: %w", err)
			}
			return "explainer result successfully processed", nil
		},
		Summarizer: fp.GetSummarizeResultHandler(taskID, subtaskID),
	}
	executor, err := fp.executor.GetExplainerExecutor(cfg)
	if err != nil {
		return "", fmt.Errorf("failed to get explainer executor: %w", err)
	}

	msgChainID, chain, err := fp.restoreChain(
		ctx, taskID, subtaskID, optAgentType, msgChainType, systemExplainerTmpl, userExplainerTmpl,
	)
	if err != nil {
		return "", fmt.Errorf("failed to restore chain: %w", err)
	}
	...
	err = fp.performAgentChain(ctx, optAgentType, msgChainID, taskID, subtaskID, chain, executor, fp.summarizer)
	if err != nil {
		return "", fmt.Errorf("failed to get task explainer result: %w", err)
	}
	...
	return explainerResult.Result, nil
}

func (fp *flowProvider) performRemediator(
	ctx context.Context,
	taskID, subtaskID *int64,
	systemRemediatorTmpl, userRemediatorTmpl, question string,
) (string, error) {
	var (
		remediatorResult tools.RemediatorResult
		optAgentType     = pconfig.OptionsTypeRemediator
		msgChainType     = database.MsgchainTypeRemediator
	)

	ctx = tools.PutAgentContext(ctx, msgChainType)
	cfg := tools.RemediatorExecutorConfig{
		TaskID:    taskID,
		SubtaskID: subtaskID,
		RemediatorResult: func(ctx context.Context, name string, args json.RawMessage) (string, error) {
			err := json.Unmarshal(args, &remediatorResult)
			if err != nil {
				return "", fmt.Errorf("failed to unmarshal result: %w", err)
			}
			return "remediator result successfully processed", nil
		},
		Summarizer: fp.GetSummarizeResultHandler(taskID, subtaskID),
	}
	executor, err := fp.executor.GetRemediatorExecutor(cfg)
	if err != nil {
		return "", fmt.Errorf("failed to get remediator executor: %w", err)
	}

	msgChainID, chain, err := fp.restoreChain(
		ctx, taskID, subtaskID, optAgentType, msgChainType, systemRemediatorTmpl, userRemediatorTmpl,
	)
	if err != nil {
		return "", fmt.Errorf("failed to restore chain: %w", err)
	}
	...
	err = fp.performAgentChain(ctx, optAgentType, msgChainID, taskID, subtaskID, chain, executor, fp.summarizer)
	if err != nil {
		return "", fmt.Errorf("failed to get task remediator result: %w", err)
	}
	...
	if agentCtx, ok := tools.GetAgentContext(ctx); ok {
		fp.putAgentLog(
			...
			agentCtx.CurrentAgentType,
			question,
			remediatorResult.Result,
			taskID,
			subtaskID,
		)
	}

	return remediatorResult.Result, nil
}

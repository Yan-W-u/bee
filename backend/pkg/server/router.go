...
func NewRouter(
	...
	swarmService := services.NewSwarmService(orm)
	providerPresetService := services.NewProviderPresetService(orm)
	...
	privateGroup.Use(authMiddleware.AuthTokenRequired)
	{
		...
		setSwarmGroup(privateGroup, swarmService)
		setProviderPresetsGroup(privateGroup, providerPresetService)
	}
...
func setSwarmGroup(parent *gin.RouterGroup, svc *services.SwarmService) {
	swarmGroup := parent.Group("/swarm")
	{
		swarmGroup.GET("/", svc.GetSwarmConfigs)
		swarmGroup.POST("/", svc.SaveSwarmConfigs)
	}
}

func setProviderPresetsGroup(parent *gin.RouterGroup, svc *services.ProviderPresetService) {
	presetsGroup := parent.Group("/provider-presets")
	{
		presetsGroup.GET("/", svc.ListProviderPresets)
		presetsGroup.POST("/", svc.CreateProviderPreset)
		presetsGroup.PUT("/:presetID", svc.UpdateProviderPreset)
		presetsGroup.DELETE("/:presetID", svc.DeleteProviderPreset)
	}
}

...
// ExplainerAction defines the question for the vulnerability explanation specialist agent.
type ExplainerAction struct {
	Question string `json:"question" jsonschema:"required" jsonschema_description:"Technical-channel payload — task description for the vulnerability explanation specialist to analyze a discovered vulnerability, explain root cause, assess impact / CVSS, and produce an understandable explanation for developers and stakeholders. Always written in English; never translated."`
	Message  string `json:"message" jsonschema:"required,title=Explainer action message" jsonschema_description:"Engagement-log entry — a 1-2 short sentence running commentary summarizing the explanation request. Written in the engagement language declared by your system prompt."`
}

// ExplainerResult is the structured result returned by the Explainer agent.
type ExplainerResult struct {
	Result  string `json:"result" jsonschema:"required,title=Explainer result description" jsonschema_description:"Technical-channel payload — fully detailed vulnerability explanation report with root cause, impact, CVSS assessment, attack chain, and developer-oriented summary. Always written in English; never translated."`
	VulnID  string `json:"vuln_id,omitempty" jsonschema:"title=Vulnerability ID" jsonschema_description:"Optional CVE or internal vulnerability identifier for traceability."`
	Severity string `json:"severity,omitempty" jsonschema:"title=Severity level,enum=Critical,enum=High,enum=Medium,enum=Low,enum=Info" jsonschema_description:"Overall severity rating for the vulnerability."`
	Message string `json:"message" jsonschema:"required,title=Explainer result message" jsonschema_description:"Engagement-log entry — a 1-2 short sentence running commentary with the result. Written in the engagement language declared by your system prompt."`
}

// RemediatorAction defines the question for the remediation specialist agent.
type RemediatorAction struct {
	Question string `json:"question" jsonschema:"required" jsonschema_description:"Technical-channel payload — task description for the remediation specialist to generate a precise, actionable fix for a discovered vulnerability, including code patches, configuration changes, dependency upgrades, and temporary mitigations. Always written in English; never translated."`
	VulnID   string `json:"vuln_id,omitempty" jsonschema:"title=Vulnerability ID" jsonschema_description:"Optional CVE or internal vulnerability identifier to associate the remediation with."`
	Message  string `json:"message" jsonschema:"required,title=Remediator action message" jsonschema_description:"Engagement-log entry — a 1-2 short sentence running commentary summarizing the remediation request. Written in the engagement language declared by your system prompt."`
}

// RemediatorResult is the structured result returned by the Remediator agent.
type RemediatorResult struct {
	Result     string `json:"result" jsonschema:"required,title=Remediator result description" jsonschema_description:"Technical-channel payload — fully detailed remediation report with step-by-step fix instructions, code patches, configuration changes, verification steps, and complexity/risk assessment. Always written in English; never translated."`
	VulnID     string `json:"vuln_id,omitempty" jsonschema:"title=Vulnerability ID" jsonschema_description:"CVE or internal vulnerability identifier this remediation addresses."`
	Complexity string `json:"complexity,omitempty" jsonschema:"title=Fix complexity,enum=Trivial,enum=Simple,enum=Moderate,enum=Complex,enum=Architectural" jsonschema_description:"Estimated complexity level for applying the remediation."`
	Message    string `json:"message" jsonschema:"required,title=Remediator result message" jsonschema_description:"Engagement-log entry — a 1-2 short sentence running commentary with the result. Written in the engagement language declared by your system prompt."`
}

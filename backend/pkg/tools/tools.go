...
type ExplainerExecutorConfig struct {
	TaskID          *int64
	SubtaskID       *int64
	ExplainerResult ExecutorHandler
	Summarizer      SummarizeHandler
}

type RemediatorExecutorConfig struct {
	TaskID            *int64
	SubtaskID         *int64
	RemediatorResult  ExecutorHandler
	Summarizer        SummarizeHandler
}
...
type FlowToolsExecutor interface {
	...
	GetExplainerExecutor(cfg ExplainerExecutorConfig) (ContextToolsExecutor, error)
	GetRemediatorExecutor(cfg RemediatorExecutorConfig) (ContextToolsExecutor, error)
...
func (fte *flowToolsExecutor) GetExplainerExecutor(cfg ExplainerExecutorConfig) (ContextToolsExecutor, error) {
	if cfg.ExplainerResult == nil {
		return nil, fmt.Errorf("explainer result handler is required")
	}
	...
	ce := &customExecutor{
		...
		store:     fte.store,
		definitions: []llms.FunctionDefinition{
			registryDefinitions[ExplainerResultToolName],
			registryDefinitions[TerminalToolName],
			registryDefinitions[FileToolName],
		},
		handlers: map[string]ExecutorHandler{
			ExplainerResultToolName: cfg.ExplainerResult,
			TerminalToolName:        term.Handle,
			FileToolName:            term.Handle,
		},
		barriers: map[string]struct{}{
...
func (fte *flowToolsExecutor) GetRemediatorExecutor(cfg RemediatorExecutorConfig) (ContextToolsExecutor, error) {
	if cfg.RemediatorResult == nil {
		return nil, fmt.Errorf("remediator result handler is required")
	}
	...
	ce := &customExecutor{
		...
		store:     fte.store,
		definitions: []llms.FunctionDefinition{
			registryDefinitions[RemediatorResultToolName],
			registryDefinitions[TerminalToolName],
			registryDefinitions[FileToolName],
		},
		handlers: map[string]ExecutorHandler{
			RemediatorResultToolName: cfg.RemediatorResult,
			TerminalToolName:         term.Handle,
			FileToolName:             term.Handle,
		},
		barriers: map[string]struct{}{

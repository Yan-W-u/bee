...
import { Hexagon, Cpu, Key, Link, Save, TestTube, ChevronDown, ChevronUp, CheckCircle, XCircle, Activity, Zap, Shield, Search, Code, Lightbulb, Brain, Package, FileText, MessageSquare, RefreshCw, Layers, Eye, EyeOff } from 'lucide-react'
...
export default function Swarm() {
  ...
  const toggleExpand = (id:string) => setAgents(p=>p.map(a=>a.id===id?{...a,expanded:!a.expanded}:a))
  const toggleKey = (id:string) => setShowKeys(p=>({...p,[id]:!p[id]}))
  const updateAgent = (id:string,field:keyof Agent,value:string) => setAgents(p=>p.map(a=>a.id===id?{...a,[field]:value}:a))
  const handleProviderChange = (id:string,pv:string) => {
  ...
  const handleTest = (a:Agent) => { if(!a.apiKey){toast.error(t('swarm.fillKey'));return} toast.success(a.name+' '+t('swarm.testSuccess')) }
  const handleSave = (a:Agent) => { if(!a.apiKey){toast.error(t('swarm.fillKey'));return} setAgents(p=>p.map(x=>x.id===a.id?{...x,expanded:false,status:'configured'}:x)); toast.success(a.name+' '+t('swarm.saved')) }
...
Write-Output "Written: $len bytes"
